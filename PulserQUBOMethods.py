...
def create_distances_matrix(
    distances, node0: Optional[int] = None, nodeN1: Optional[int] = None
):
    """
...
def create_QUBO_matrix(
    ...
    R,
    distances_matrix,
    active_constraints: Optional[List[bool]] = None,
):
    """
...
def constraint_1_check(solution_array, p):
    if np.sum(solution_array) != p:
        print(
            "The lenght of the path is ",
            np.sum(solution_array),
            "but the p value is ",
            p,
        )
        return False
...
def constraint_2_check(adjacency_matrix, N):
    ...
    print(
        "Constraint 2 passed: All nodes (apart from 0 and N-1) have 0 or 2 connections."
    )
    return True
...
def draw_graph(adjacency_matrix, distance_matrix):
    ...
    for i in range(N):
        for j in range(
            i + 1, N
        ):  # Solo recorrer la parte superior para evitar duplicados en grafos no dirigidos
