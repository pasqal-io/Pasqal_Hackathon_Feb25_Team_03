...
class Bus_lines_solver:
    ...
    def __init__(
        ...
        N: int,
        M: int,
        restrictions_multipliers: Optional[List[float]] = None,
        seed_number: Optional[int] = None,
        numberOfStopsPerLine: Optional[int] = None,
    ) -> None:
        self.N = N
        self.M = M
        ...
        self.stops_graph, self.distances_matrix = self._create_complete_graph(
            seed=seed_number
        )
    ...
    def _create_QUBO(self):
        ...
        connections = Array.create(
            "item", shape=(self.N * self.N, self.M), vartype="BINARY"
        )
        H_cost = sum(
            self.distances_matrix[i, i] * connections[i, l]
        ...
        for index, restriction in enumerate(list_of_restrictions):
            restrictions_hamiltonian, lambda_expression = self._callRestrictionMethod(
    ...
    def solve_QUBO(self):
        feed_dict = {
            ...
            "lmbIfLineArrivesToStopItDepartsFromItRestriction": self.restrictions_multipliers[
                3
            ],
        ...
        for i, j, l in self.active_edges:
            if self.stops_graph.has_edge(i, j):
                subgraph.add_edge(
                    ...
                    color=self.colors[l],
                    label=f"Line {l}",
                    weight=self.distances_matrix[self.N * i + j, self.N * i + j],
                )
    ...
    def __stopsEquallyDistributedRestriction(self, connections: pyqubo.Array):
        ...
        for l in range(self.M):
            ...
            H += Constraint(
                (terms - self.p) ** 2, label=f"EquallyDistributedRestriction_line{l}"
            )
    ...
    def __oneArrivalAtMostRestriction(self, connections: pyqubo.Array):
        s2 = {
            (l, i): Binary(f"s2_{l}_{i}") for l in range(self.M) for i in range(self.N)
        }
        H2 = 0
        ...
        for l in range(self.M):
            for i in range(self.N):
                ...
                H2 += Constraint(
                    (s2[l, i] + terms2 - 1) ** 2,
                    label=f"OneArrivalAtMostRestriction_line{l}_stop{i}",
                )
    ...
    def __oneDepartureAtMostRestriction(self, connections: pyqubo.Array):
        s3 = {
            (l, i): Binary(f"s3_{l}_{i}") for l in range(self.M) for i in range(self.N)
        }
        H3 = 0
        ...
        for l in range(self.M):
            for i in range(self.N):
                ...
                H3 += Constraint(
                    (s3[l, i] + terms3 - 1) ** 2,
                    label=f"OneDepartureAtMostRestriction_line{l}_stop{i}",
                )
    ...
    def __closedPathsOfLenghtPRestriction(self, connections: pyqubo.Array):
        ...
        for l in range(self.M):
            terms4 = sum(
                self._apply_power_with_reshape(connections[:, l], self.p)[
                    k * (self.N + 1)
                ]
                for k in range(self.N)
            )
            H4 += Constraint(
                (terms4 - self.p) ** 2, label=f"ClosedPathsOfLenghtPRestriction_line{l}"
            )
    ...
    def __noClosedPathsOfLenghtLessThanPRestriction(self, connections: pyqubo.Array):
        ...
        for l in range(self.M):
            terms5 = sum(
                self._apply_power_with_reshape(connections[:, l], n)[k * (self.N + 1)]
                for k in range(self.N)
                for n in range(self.p)
            )
            H5 += Constraint(
                terms5**2, label=f"NoClosedPathsOfLenghtLessThanPRestriccion_line{l}"
            )
    ...
    def __globalPathOfLenghtNRestriction(self, connections: pyqubo.Array):
        ...
        terms = sum(
            self._apply_power_with_reshape(matrixTerms, self.N)[k * (self.N + 1)]
            for k in range(self.N)
        )
        ...
        for l in range(self.M):
            for i in range(self.N):
                for j in range(self.N):
                    ...
                    H7 += Constraint(
                        (connections[i * self.N + j, l] - terms7) ** 2,
                        label=f"IfLineArrivesToStopItDepartsFromItRestriction_line{l}_stop{i}_stop{j}",
                    )
    ...
    def __allStopsAreVisited(self, connections: pyqubo.Array):
        ...
        for j in range(self.N):
            terms8 = sum(
                connections[i * self.N + j, l]
                for i in range(self.N)
                for l in range(self.M)
            )
            H8 += Constraint(
                (terms8 - 1 - s41[j] - 2 * s42[j]) ** 2,
                label=f"AllStopsAreVisitedRestriction_stop{j}",
            )
    ...
    def draw_stops_graph(self) -> plt.Figure:
        ...
        nx.draw_networkx_nodes(
            self.stops_graph, pos, node_color="lightblue", node_size=700
        )
    ...
    def average_distance_between_stops(self):
        ...
        for i in range(self.N):
            for j in range(i + 1, self.N):
                mean_distance = (
                    self.distances_matrix[i * self.N + j, i * self.N + j]
                    + self.distances_matrix[i + self.N * j, i + self.N * j]
                ) / 2
